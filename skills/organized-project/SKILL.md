...
description: Fetches a work project and its background links from Organized. Use when invoked for an Organized project, when a project identifier must be resolved from WORK_PROJECT_ID, or when another skill needs project title and description as background.
---

# Organized Project

Fetch one project from Organized's `work_resources` collection and return a small background record. Do not begin project work.

## Input

Use the project identifier supplied by the caller. When it is omitted, read `WORK_PROJECT_ID`. Stop with a clear error when neither is available.
...
1. Call `quick_query_collection` with:
   ...
   - `filter`: `{"resource_type":"project","resource_key":"project/<project-id>"}`
   - `limit`: `2`
...
Start with `WORK_PROJECT_CONTEXT_ID` when set; otherwise use the project's `context_id`. For each context identifier:
